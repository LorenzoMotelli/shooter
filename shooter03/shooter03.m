...
% LAUNCH THIS FUNCTION SPECIFING THE SERIAL PORT
% e.g. shooter03('COM1')
function [] = shooter03(serialPortExt)
...
% Mie modifiche
serialPort = 'COM8'; % '/dev/tty.usbserial-210183AA0D131'
HEADER = [hex2dec('FE') hex2dec('B0')] ;
FOOTER = [hex2dec('0B') hex2dec('EF')];
input_data = [];
x_joy = 0;
y_joy = 0;
but_joy = 0;
...
NO_FILL = true; %set to true if game lags. ships are just outlines.
INVINCIBLE = true; %hero doesn't die if shield goes below 0
...
%screen.
  function showIntro
    ...
    t(3) = text(x, useY, 'use joystick to move and shoot');
...
%run each frame.
  function moveHero
    if ~gameOver
      ...
      %tempV = mousePos - heroCenter;    % Inserisco in tempV i valori diretti del joystick
      
      tempV = [x_joy, y_joy] .* (HERO_SPEED / JOY_MAX_VALUE);
      ...
      %  tempV = tempV ./ magnitude;
        ...
        heroPos = heroPos + (tempV * 1.5);% * heroSpeed);
      %else
    ...
    if ~gameOver %no shooting if your dead
      ...
      elseif but_joy
        ...
        if energy - ENERGY_DRAIN >= 0 %only shoot if have enough energy
          ...
          % Scrivo qui la modifica per il led 
...
%if were in the middle of a bot1 formation, make another bot1 andelseif mouseDown
...
%AutoCallback for new data
    function serial_fcn(src, event)
        s = src;
        if s.BytesAvailable
            input_data = [input_data ; fread(s, s.BytesAvailable, 'uint8')];
            %newData = fread(s, s.BytesAvailable);
        end
            
        % 1B HEADER + 3B DATA + 1B FOOTER
        
        while length(input_data) > 7
           
            if  input_data(1) == HEADER(1) && ...
                input_data(2) == HEADER(2) && ...
                input_data(6) == FOOTER(1) && ...
                input_data(7) == FOOTER(2)
                ...
                %input_data(2)
                x_joy = input_data(3) - x_zero;
                %input_data(3)
                y_joy = input_data(4) - y_zero;
                but_joy = input_data(5);
                
                input_data = [];%input_data(6:end);
            else
    ...
    %AutoCallback for sending led status
    function send_led_fcn(src, event)
        rgb = [0 0 0];
        
        
        fwrite(serialHandler, HEADER(1), 'uint8');
        fwrite(serialHandler, HEADER(2), 'uint8');
        
        energyTemp = energy - 0.02;
        if energyTemp < 0
            energyTemp = 0;
        end
        
        if superShoot > 1 % Se è abilitato il super shoot cambio colore
            rgb(1) = energyTemp * 255;
        else
            rgb(2) = energyTemp * 255;
        end 
        fwrite(serialHandler, rgb(1), 'uint8');
        fwrite(serialHandler, rgb(2), 'uint8');
        fwrite(serialHandler, rgb(3), 'uint8');
        
        fwrite(serialHandler, FOOTER(1), 'uint8');
        fwrite(serialHandler, FOOTER(2), 'uint8');
    end
...
serialHandler = serial(serialPort);
serialHandler.BaudRate = 115200;
serialHandler.BytesAvailableFcnCount = 7;
serialHandler.BytesAvailableFcnMode = 'byte';
serialHandler.BytesAvailableFcn = @serial_fcn;
